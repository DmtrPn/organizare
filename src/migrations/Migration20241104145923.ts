...
export class Migration20241104145923 extends Migration {
  ...
  override async up(): Promise<void> {
    ...
    this.addSql(`create table "organization_users" ("organization_model_organization_id" uuid not null, "user_model_user_id" uuid not null, constraint "organization_users_pkey" primary key ("organization_model_organization_id", "user_model_user_id"));`);

    this.addSql(`alter table "organization_users" add constraint "organization_users_organization_model_organization_id_foreign" foreign key ("organization_model_organization_id") references "organization" ("organization_id") on update cascade on delete cascade;`);
    this.addSql(`alter table "organization_users" add constraint "organization_users_user_model_user_id_foreign" foreign key ("user_model_user_id") references "users" ("user_id") on update cascade on delete cascade;`);
  }
